...
namespace MultiShop.Web.UI.Areas.Admin.Controllers
{
    ...
    public class CategoryController : Controller //Adminin
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responsoMessage = await client.DeleteAsync("https://localhost:7070/api/Categories?id=" +id);
        ...
        public async Task<IActionResult> UpdateCategory(string id)
        {
            ViewBag.v1 = "Ana Sayfa";
            ViewBag.v2 = "Kategoriler";
            ViewBag.v3 = " Kategori Girişi";
            @ViewBag.v0 = "Kategori İşlemleri";

            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7070/api/Categories/id?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
